...
def fit_scaler(scaler_filename, data):
    dims = data.shape
    scaler = MinMaxScaler()
    bidim_data = data.reshape(dims[0], -1)
...
def plot_sample(data):
    plt.plot(np.arange(0, data.shape[1]), data[0, :, :])
    plt.show()
...
if __name__ == '__main__':
    ...
    print('\n')
    for i in range(12):
        print('class: ', i, '\tsamples: ', counts_train[i]+counts_test[i],'\tsplit: ', counts_train[i]/(counts_train[i]+counts_test[i]))
    plot_sample(x_train)
    plot_sample(x_test)
